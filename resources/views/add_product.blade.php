      ...
      <div class="bg-light p-5 rounded">
        ...
        <form action="{{ route('products.store') }}" method="post" class="px-md-5" enctype="multipart/form-data">
          ...
          <div class="form-group mb-3 row">
            ...
            <div class="col-md-10">
              <textarea cols="30" rows="5" class="form-control py-2" name="description">{{ old('description') }}</textarea>
              @error('description')
                <div class="alert alert-warning">{{ $message }}</div>
              @enderror
            </div>
          ...
          <div class="form-group mb-3 row">
            ...
            <div class="col-md-10">
              <input type="checkbox" name="published" class="form-check-input" style="padding: 0.7rem;" value="1" @checked(old('published')) />
              @error('published')
                <div class="alert alert-warning">{{ $message }}</div>
              @enderror
            </div>
          ...
          <div class="text-md-end">
            <button class="btn mt-4 btn-secondary text-white fs-5 fw-bold border-0 py-2 px-md-5">Add Product</button>
          </div>
        </form>
      </div>
    </div>
